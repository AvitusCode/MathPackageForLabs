...
RC myv::MyVector::scale(double multiplier)
{
	...
	// Сначала проверяем данные, потом сохраняем их. Чтобы оптимизировать код по скорости - можно завести доп массив и сохранять данные, а уже потом просто перекопировать их
